...
int main(){
	...
	while(rc_get_state()!=EXITING){
		...
		if(rc_get_state()==RUNNING){
			...
			int left_encoder = -rc_encoder_eqep_read(1);
			int right_encoder = rc_encoder_eqep_read(2);
			printf("Left encoder: %d\n", left_encoder);
			printf("Right encoder: %d\n", right_encoder);

			rc_nanosleep(1E3);
			// t = rc_nanos_since_boot();
			// float left_encoder = rc_encoder_eqep_read(1);
   //  		float encoder_dt  = (float)(t - t_0) / 1E9;
   //  		w_left = 2*3.14*(left_encoder)/(encoder_dt*979.62);
